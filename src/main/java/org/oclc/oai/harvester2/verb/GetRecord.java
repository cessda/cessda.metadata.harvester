...
public final class GetRecord extends HarvesterVerb {
     ...
     */
    public static GetRecord instance(HttpClient httpClient, URI baseURL,
                                     String identifier, String metadataPrefix)
            throws IOException, SAXException {
        var requestURL = URI.create(baseURL
                                    + "?verb=GetRecord"
                                    + "&identifier="
                                    + identifier
                                    + "&metadataPrefix="
    ...
    public RecordHeader getHeader() {
        var recordHeader = getDocument().getElementsByTagNameNS(
     ...
     *
     * @return the metadata section of the document, or an empty
     *     optional if metadata was not returned.
     */
    public Optional<Element> getMetadata() {
        var metadataElements = getDocument().getElementsByTagNameNS(
            OAI_2_0_NAMESPACE, "metadata");
