...
public final class ListSets extends HarvesterVerb implements Resumable {
    ...
    public static ListSets instance(HttpClient httpClient, URI baseURL)
    ...
    private static URI getRequestURL(URI baseURL, String resumptionToken) {
        return URI.create(baseURL + "?verb=ListSets" + "&resumptionToken="
                + URLEncoder.encode(resumptionToken, StandardCharsets.UTF_8));
    }

    /**
     * Returns a list of sets found in the response. The returned list
     * is unmodifiable.
     */
    public List<String> getSets() {
        var nl = getDocument().getElementsByTagNameNS(OAI_2_0_NAMESPACE,
                                                      "setSpec");
