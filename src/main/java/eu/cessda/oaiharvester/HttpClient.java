...
public class HttpClient {
    ...
    public HttpClient(HarvesterConfiguration harvesterConfiguration) {
        // TK added default timeout for dataverses taking too long to
        // respond / stall
    ...
    private static long parseRetryAfterHeader(HttpResponse<?> response) {
        ...
        if (retryAfterHeader.isPresent()) {
            ...
            try {
                var httpDate = ZonedDateTime.parse(
     ...
     * @return the response.
     * @throws IOException if an IO error occurred when sending the
     *     response and the maximum retries have been exceeded.
     * @throws InterruptedException if the operation is interrupted.
     */
    // Any other way of implementing this logic is more complicated
    @SuppressWarnings({ "java:S3776", "java:S6212" })
    private <T> HttpResponse<T> performRequest(
        ...
        while (true) {
            try {
                ...
                if (responseCode < 400) {
                    ...
                    // Try to parse the Retry-After header, fall back
                    // to default behavior if the header is not
                    // present
                ...
                currentException = new HTTPException(httpRequest.uri(),
                                                     response.statusCode());
                if (responseCode != 429 && responseCode < 500) {
                    // 400 response codes, apart from 429, are caused
                    // by client errors, do not retry
                    break;
                }
            } catch (IOException e) {
                currentException = e;
