...
public class IOUtilities {
    ...
    // This application is not a server application.
    @SuppressWarnings("java:S5164")
    private final ThreadLocal<Transformer> transformerThreadLocal
     ...
     * @throws DirectoryCreationFailedException if the directory cannot be
     *     created.
     * @return the created directory.
     */
    static Path createDestinationDirectory(Path destinationDirectory,
    ...
    /**
     * Create metadata describing the repository in the given
     * directory. The filename created is metadata.json.
     *
     * @param directory the directory to write to.
     * @param repository the repository to write metadata for.
     * @param metadataFormat the metadata format for this harvest.
     */
    static void createMetadata(Path directory, Repo repository,
                               Repo.MetadataFormat metadataFormat)
            throws MetadataCreationFailedException {
        ...
        try (var outputStream = Files.newOutputStream(
                directory.resolve("pipeline.json"))) {
     ...
     *     whilst writing the source.
     * @param source the XML source.
     * @param destination the {@link Path} to write to.
     */
    void writeDomSource(Source source, Path destination)
    ...
    /**
     * Remove any records that are present in the destination
     * directory, but are not declared in the repository.
    ...
    static int deleteOrphanedRecords(Repo repo,
                                     Collection<RecordHeader> recordHeaders,
                                     Path destinationPath) {
        // Track the amount of files that were deleted
        int filesDeleted = 0;

        try (var stream = Files.newDirectoryStream(destinationPath, "*.xml")) {
            // Collect encountered identifiers to a HashSet, this will
            // be used for comparisons
            var recordIdentifiers = new HashSet<String>(
                (int) (recordHeaders.size() / 0.75F), 0.75F);
            for (var recordHeader : recordHeaders) {
                var identifier = recordHeader.identifier();
                recordIdentifiers.add(URLEncoder.encode(identifier, UTF_8)
                                      + ".xml");
            }

            // Select records not discovered by the repository
            for (var fileName : stream) {
                if (!recordIdentifiers.contains(
