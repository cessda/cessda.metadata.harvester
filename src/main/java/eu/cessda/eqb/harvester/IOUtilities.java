...
public class IOUtilities {
     ...
     */
    static Path createDestinationDirectory(Path destinationDirectory, Path repositoryDirectory)
    ...
    /**
     * Create metadata describing the repository in the given directory. The
     * filename created is metadata.json.
     * 
     * @param directory      the directory to write to.
     * @param repository     the repository to write metadata for.
     * @param metadataFormat the metadata format for this harvest.
     */
    static void createMetadata(Path directory, Repo repository, Repo.MetadataFormat metadataFormat)
     ...
     * @throws TransformerException if an unrecoverable error occurs whilst writing
     *                              the source.
    ...
    /**
     * Remove any records that are present in the destination directory, but are not
    ...
    static int deleteOrphanedRecords(Repo repo, Collection<RecordHeader> recordHeaders, Path destinationPath) {
        ...
        try (var stream = Files.newDirectoryStream(destinationPath, "*.xml")) {
            // Collect encountered identifiers to a HashSet, this will be used for
            // comparisons
